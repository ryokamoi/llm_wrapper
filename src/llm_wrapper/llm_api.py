...
def preprocess_parameters(model_name: str, updated_parameters: dict) -> dict:
    ...
    if is_open_model(model_name):
        ...
        parameters = dict(updated_parameters, model=model_name)
        ...
        if "max_tokens" not in updated_parameters.keys():
            raise ValueError("'max_tokens' must be included in the updated_parameters.")
...
def llm_api(model_name: str, prompt: str, updated_parameters: dict={},
    ...
    if not overwrite_cache:
        ...
        if len(cached_output) > 0:
            ...
            # if the above conditions are not applicable, the cache_output includes a proper cache
            if len(cached_output) > 0:
    ...
    if is_openai_model(model_name):  # openai models
        ...
        # call api
        output = openai_text_api(
            mode="chat", parameters=get_chat_parameters(prompt=prompt, parameters=parameters),
            organization=openai_organization,
        )
        response = output["response"]["choices"][0]["message"]["content"]
    elif is_open_model(model_name):
